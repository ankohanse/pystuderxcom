...
class AsyncXcomFactory:
    ...
    async def create_dataset(voltage: str) -> XcomDataset:
        ...
        """
        async with aiofiles.open(XcomDataset.PATH_120V, "r", encoding="UTF-8") as file_120vac:
            text_120vac = await file_120vac.read()
        async with aiofiles.open(XcomDataset.PATH_240V, "r", encoding="UTF-8") as file_240vac:
    ...
    @staticmethod
    async def parse_package(f: asyncio.StreamReader, verbose=False) -> XcomPackage:
    ...
    @staticmethod
    async def parse_package_bytes(buf: bytes, verbose=False) -> XcomPackage:
        reader = asyncio.StreamReader()
        reader.feed_data(buf)

        package, _ = await AsyncXcomFactory.parse_package(reader, verbose)
        return package
