...
class XcomApiBase(StuderApi):
    ...
    def request_virtual(self, parameter: StuderDatapoint, device: StuderDiscoveredDevice|int|str=None, retries = None, timeout = None, verbose=False):
        ...
        match parameter.nr:
            ...
            case 99020 | 99021 | 99022 | 99023:
                msg = self.request_message(index=0, retries=retries, timeout=timeout, verbose=verbose)
                if msg:
                    match parameter.nr:
    ...
    def request_values(self, request_data: StuderValueSet, retries = None, timeout = None, verbose=False) -> StuderValueSet:
        ...
        for req_virtual in req_virtuals:
            try:
                error = None
                value = self.request_virtual(req_virtual.datapoint, req_virtual.address, retries=retries, timeout=timeout, verbose=verbose)
    ...
    def request_message(self, index:int = 0, retries = None, timeout = None, verbose=False) -> XcomMessage:
        ...
        if self._msg_set is None:
            self._msg_set = XcomMessageSet.get_instance()
    ...
    def _parse_package(self, f: BufferedReader, timeout:float=REQ_TIMEOUT, verbose=False) -> XcomPackage:
    ...
    def _parse_package_bytes(self, buf: bytes, timeout:float=REQ_TIMEOUT, verbose=False) -> XcomPackage:
        reader = SyncReader(buf)
        return self._parse_package(reader, timeout, verbose)
